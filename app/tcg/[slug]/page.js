...
export default async function TcgPage({ params }) {
  ...
  return (
    <main><PublicHeader /><Link href="/tcgs" className="back-link">← All TCGs</Link>
      <section className={`catalog-heading tcg-heading-${tcg.slug}`}><div><p className="eyebrow">Trading card game</p><h1>{headerLogo ? <img className="tcg-official-logo" src={headerLogo} alt={tcg.name} /> : tcg.name}</h1><p>{tcg.description}</p></div></section>
